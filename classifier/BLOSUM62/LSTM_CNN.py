...
def get_LSTM_CNN_model(train_X):
    ...
    Lstm1 = LSTM(64, return_sequences=True)(lstminputs)
...
# 2、数据处理(准备训练数据)
data_=pd.read_csv(r'../../feature/BLOSUM62/BLOSUM62_Train.csv')
data=np.array(data_)
data = data[:,1:]
print(data.shape)
...
# X_ = min_max_scaler.fit_transform(data)
...
history = lstm_model.fit(train_X, train_y, batch_size=512, epochs=500, validation_split=0.3, verbose=2)
...
for train, test in kfold.split(train_X, train_y):
    ...
    lstm_model.fit(train_X[train], train_y[train], batch_size=512,epochs=500, verbose=0)
    # evaluate the model
...
dataframe.to_csv("BLOSUM62_LSTM_CNN.csv",index=False,sep=',')
