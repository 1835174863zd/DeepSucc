...
def get_LSTM_CNN_model(train_X):
    ...
    conv1 = Conv1D(filters=16, strides=1, kernel_size=3, padding='valid', kernel_initializer='random_normal')(
        cnninputs)
    conv1 = LeakyReLU(alpha=0.01)(conv1)
    conv1 = Dropout(0.5)(conv1)
...
history = lstm_model.fit(train_X,train_y,batch_size=128,epochs=150,validation_split=0.3,verbose=2)
...
for train, test in kfold.split(train_X, train_y):
    ...
    lstm_model.fit(train_X[train], train_y[train], batch_size=128,epochs=91, verbose=0)
    # evaluate the model
...
dataframe.to_csv("CKSAAP_LSTM_CNN.csv",index=False,sep=',')
