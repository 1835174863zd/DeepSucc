...
BLOSUM62_data = pd.read_csv(r'../../feature/BLOSUM62/BLOSUM62_Train.csv')
BLOSUM62_data = np.array(BLOSUM62_data)
BLOSUM62_data = BLOSUM62_data[:,1:]
BLOSUM62_data = scale(BLOSUM62_data)#标准化


ACF_data = pd.read_csv(r'../../feature/ACF/ACF_Train.csv',header=None)
ACF_data = np.array(ACF_data)
ACF_data = scale(ACF_data)

onehot_data = pd.read_csv(r'../../feature/onehot/onehot_Train.csv')
onehot_data = np.array(onehot_data)
onehot_data = onehot_data[:,1:]
onehot_data = onehot_data.astype('float64')

aaindex_data = pd.read_csv(r'../../feature/AAindex/aaindex_Train.csv')
aaindex_data = np.array(aaindex_data)
aaindex_data = aaindex_data[:,1:]
aaindex_data = aaindex_data.astype('float64')

CKSAAP_data = pd.read_csv(r'../../feature/CKSAAP/CKSAAP_Train.csv',header=None)
