...
def get_RNN_model(train_X):
    ...
    #model.add(Conv1D(filters=128, strides=10, kernel_size=15, padding='same'))
...
#3、训练模型
lstm_model = get_LSTM_model(train_X)
history = lstm_model.fit(train_X,train_y,batch_size=128,epochs=50,validation_split=0.3,verbose=2)
# plot history
...
for train, test in kfold.split(train_X, train_y):
    ...
    lstm_model.fit(train_X[train], train_y[train], batch_size=50,epochs=39, verbose=0)
    # evaluate the model
...
dataframe.to_csv("ACF_LSTM_CNN.csv",index=False,sep=',')
