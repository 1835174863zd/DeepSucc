...
def get_CNN_LSTM_model(train_X):
    ...
    conv1 = Conv1D(filters=32, strides=20, kernel_size=20, padding='valid', kernel_initializer='he_normal')(inputs)
...
# 2、数据处理(准备训练数据)
data_=pd.read_csv(r'../../feature/AAindex/aaindex_Train.csv')
data=np.array(data_)
data = data[:,1:]
print(data.shape)
#[m1,n1]=np.shape(data)
...
history = lstm_model.fit(train_X,train_y,batch_size=128,epochs=600,validation_split=0.3,verbose=2)
...
for train, test in kfold.split(train_X, train_y):
    ...
    lstm_model.fit(train_X[train], train_y[train], batch_size=128,epochs=600, verbose=0)
    # evaluate the model
...
dataframe.to_csv("AAindex_CNN_LSTM.csv",index=False,sep=',')
