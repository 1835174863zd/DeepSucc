...
from sklearn.preprocessing import scale, StandardScaler, binarize
from keras.models import Sequential
from keras.layers import Input, Activation, Dense, Dropout, Bidirectional, Conv1D, BatchNormalization, LeakyReLU, \
...
def get_CNN_model(train_X):
    ...
    conv1 = Conv1D(filters=32, strides=400, kernel_size=400, padding='valid', kernel_initializer='random_normal')(inputs)
...
# 2、数据处理(准备训练数据)
data = pd.read_csv(r'../../feature/CKSAAP/CKSAAP_Train.csv',header=None)
data = np.array(data)
label1=np.ones((21767,1))
...
history = lstm_model.fit(train_X,train_y,batch_size=64,epochs=500,validation_split=0.3,verbose=2)
...
for train, test in kfold.split(train_X, train_y):
    # create model
    lstm_model = get_RNN_model(train_X)
    x_cv_tarin, y_cv_train = train_X[train], train_y[train]
    x_cv_test, y_cv_test = train_X[test], train_y[test]
    lstm_model.fit(train_X[train], train_y[train], batch_size=128,epochs=150, verbose=0)
    # evaluate the model
...
dataframe.to_csv("CKSAAP_CNN_LSTM.csv",index=False,sep=',')
