...
precisions = []
result = np.zeros((traindata.shape[0], 3))
for train, test in kfold.split(traindata, label):
    ...
    y_cv_test = y_cv_test.reshape((y_cv_test.shape[0],))
    result[test, 0] = y_cv_test
    result[test, 1] = y_pred_class
    result[test, 2] = y_pred[:, 0]
    ...
    #precisions[-1][0] = 1.0
    #print(precisions)
    pyplot.plot(recall, precision, lw=1, alpha=0.5, label='ROC fold %d(area=%0.4f)' % (count, roc_auc))
    '''
...
dataframe.to_csv("hybrid-10folds.csv",index=False,sep=',')
'''
