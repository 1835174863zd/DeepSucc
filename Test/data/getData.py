...
protein = []
sequence = []
with open('test_sequence.txt') as fp:
    lines = fp.readlines()
...
final_data = []
# 打开excel文件,获取工作簿对象
wb = openpyxl.load_workbook('Independent_data.xlsx')
# 从表单中获取单元格的内容
ws = wb.active  # 当前活跃的表单
#print(ws.cell(row=1, column=2))  # 获取第一行第二列的单元格
#print(ws.cell(row=1, column=2).value)
for i in range(3, 3234):  # 获取1,3,4,7 行第二列的值
...
Independent_data = pos_data + neg_data
print(len(pos_data),pos_number)
print(len(neg_data),neg_number)
print(len(Independent_data))
with open('Independent_test.csv', 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
    for row in Independent_data:
        writer.writerow(row)